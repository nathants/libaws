...
- `ApiList` returns only complete API identities. A page with a missing name or ID is retried within a six-attempt bound before returning an explicit incomplete-metadata error; SDK/provider errors are not outer-retried. No partial page is appended or silently dropped, and no unrelated per-API configuration is hydrated. `lib/api_test.go` covers transient/persistent incomplete second pages and denied reads; the Go API example checks CLI listing before and after teardown.
...
- EventBridge and S3 targets must match an unqualified Lambda ARN in the caller's partition/account and current region before becoming triggers. Unmatched EventBridge targets remain standalone; hide a rule only after verifying its trigger relationship, never from its name. Multiple EventBridge targets and unrepresentable S3 Lambda notifications are explicit inventory errors.
