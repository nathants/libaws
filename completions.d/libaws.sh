...
_libaws () {
    ...
    if [ "${COMP_CWORD}" = 1 ] && [ -z "${COMP_WORDS[1]}" ]; then
        ...
        elif [ ${COMP_WORDS[1]} = ec2-ip-private  ]; then COMPREPLY=($(libaws ec2-ls 2>/dev/null | awk '{print $1}' | grep "^${COMP_WORDS[2]}"))
        elif [ ${COMP_WORDS[1]} = ec2-dns         ]; then COMPREPLY=($(libaws ec2-ls 2>/dev/null | awk '{print $1}' | grep "^${COMP_WORDS[2]}"))
