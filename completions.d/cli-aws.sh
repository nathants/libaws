...
_cli_aws () {
    ...
    if   [ ${COMP_WORDS[1]} = s3-ls   ] && [ $COMP_CWORD = 2 ]; then arg=""; for i in $(seq 2 $COMP_CWORD); do arg="${arg}${COMP_WORDS[$i]}"; done; COMPREPLY=($(cli-aws s3-ls -q "$arg" 2>/dev/null | grep "^$arg"))
    elif [ ${COMP_WORDS[1]} = s3-get  ] && [ $COMP_CWORD = 2 ]; then arg=""; for i in $(seq 2 $COMP_CWORD); do arg="${arg}${COMP_WORDS[$i]}"; done; COMPREPLY=($(cli-aws s3-ls -q "$arg" 2>/dev/null | grep "^$arg"))
    elif [ ${COMP_WORDS[1]} = s3-head ] && [ $COMP_CWORD = 2 ]; then arg=""; for i in $(seq 2 $COMP_CWORD); do arg="${arg}${COMP_WORDS[$i]}"; done; COMPREPLY=($(cli-aws s3-ls -q "$arg" 2>/dev/null | grep "^$arg"))
    elif [ ${COMP_WORDS[1]} = s3-rm   ] && [ $COMP_CWORD = 2 ]; then arg=""; for i in $(seq 2 $COMP_CWORD); do arg="${arg}${COMP_WORDS[$i]}"; done; COMPREPLY=($(cli-aws s3-ls -q "$arg" 2>/dev/null | grep "^$arg"))
    elif [ ${COMP_WORDS[1]} = s3-rm-bucket   ] && [ $COMP_CWORD = 2 ]; then arg=""; for i in $(seq 2 $COMP_CWORD); do arg="${arg}${COMP_WORDS[$i]}"; done; COMPREPLY=($(cli-aws s3-ls -q "$arg" 2>/dev/null | grep "^$arg"))
        ...
        elif [ ${COMP_WORDS[1]} = lambda-api    ]; then COMPREPLY=($(find -type f 2>/dev/null | grep -Ev -e '/\.' -e '\.pyc$'  | sed s:./:: | grep "^${COMP_WORDS[2]}"))
