...
- EC2 `--init` runs as the image's login user, not root; use explicit `sudo` for privileged guest setup. `examples/complex/s3-ec2` demonstrates small Debian Spot guests with S3 input/output and automatic poweroff.
